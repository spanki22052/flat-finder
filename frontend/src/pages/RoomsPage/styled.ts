...
export const RoomItem = styled.button({
  display: 'flex',
  alignItems: 'center',
...
export const RoomItemName = styled.span({
  fontSize: 16,
  fontWeight: 700,
  color: theme.colors.text.primary,
  overflow: 'hidden',
...
export const RoomItemMeta = styled.span({
  fontSize: 12,
...
export const TabBtn = styled.button<{ $active: boolean }>((props) => ({
  flex: 1,
  padding: '10px 16px',
  borderRadius: theme.radius.sm,
  border: 'none',
  background: props.$active ? theme.colors.bg.card : 'transparent',
  color: props.$active ? theme.colors.primary : theme.colors.text.secondary,
  fontWeight: 600,
  fontSize: 14,
  cursor: 'pointer',
  boxShadow: props.$active ? theme.shadows.soft : 'none',
  transition: theme.transition,
}));
